 ...
 *     description: |
 *       Sends a **password reset link** to the provided email address if it belongs to a registered account.
 *
 *       For security reasons, this endpoint **always returns the same success message** regardless of
 *       whether the email exists — this prevents user enumeration attacks.
 *
 *       The reset link is valid for **10 minutes** and can only be used once.
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 *     responses:
 *       200:
 *         description: Request accepted (email sent if the account exists)
 *         content:
 *           application/json:
 *             example:
 *               success: true
 ...
 *     description: |
 *       Resets the user's password. The **reset token** must be passed as a query parameter (`?token=`).
 *       It is the raw token received via the email link, and it must:
 *       - Match the hashed token stored in the database
 *       - Not be expired (valid for 10 minutes from issue)
 *
 *       **Password rules:**
 *       - Minimum 8 characters
 *       - At least one uppercase letter
 ...
 *         schema:
 *           type: string
 *         description: The raw reset token received in the password-reset email link.
 *         example: a3f1bc9d2e4560f78ab12cd34ef56789a3f1bc9d2e4560f78ab12cd34ef56789
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 *               data: null
 *       400:
 *         description: Token missing, invalid, expired, or passwords don't match
 *         content:
 *           application/json:
 *             examples:
 *               missing_token:
 *                 summary: Token not provided in query
 *                 value:
 *                   success: false
 *                   message: Reset token is missing from the request.
 *               invalid_token:
...
// This file contains only Swagger/OpenAPI JSDoc annotations.
// No runtime code is exported — it is picked up by swagger-jsdoc via glob.
export {};
